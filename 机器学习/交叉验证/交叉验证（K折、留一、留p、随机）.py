...
# 参考：https://blog.csdn.net/rocling/article/details/93717335?
...
print('归一化后准确度:',clf.score(X_test_transformed, y_test))  # 计算测试集的度量值（准确度）
print('\n')
...
print("交叉验证Accuracy: %0.2f (+/- %0.2f)" % (scores.mean(), scores.std() * 2))  # 获取置信区间。（也就是均值和方差）
print('\n')
...
sorted(scores.keys())
print('多种度量测试结果：', scores)  # scores类型为字典。包含训练得分，拟合次数， score-times （得分次数）
print('\n')
